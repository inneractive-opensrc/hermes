...
object ScalaJsonProducer extends App with Logging {
  ...
  def startProducer(implicit config: HermesConfig) = {
    ...
    for (i <- 1 to numRecordToSend) {
      Seq(("creative1","100"),("creative2","0.01"),("creative3","0")) foreach { case (creative, price) =>
        val newJson = json.replace("#CreativeId#",creative).replace("#Gross#",price)
        val record = new ProducerRecord[String, String]("AGGREGATION_EVENT_JSON", i.toString, newJson)
